...
- MariaDB credentials stay in memory unless **Remember login for this workspace** is enabled. Opted-in logins are encrypted using the current Windows account's data protection, restored on opening the workspace, and validated before database tools use them. Database tabs share a successful validation; changing tabs or selecting another database does not repeat the login check. Changed credentials, a new app/workspace session, service changes, or a connection failure require validation again. **Change Credentials** explicitly rechecks the login. Passwords are not written to browser storage. Uncheck the option to delete the saved copy; a failed save/delete is shown in the connection card. A connection string written to `server.cfg` is still a plaintext secret in that file.
...
The optional **mysql_native_password** control in the account forms selects a database authentication method for connectors that require it. It does not optimize the game server or grant permissions. Leave it unchecked unless your connector requires this method; existing accounts retain their authentication method by default. Changing an existing account's authentication requires a new password and confirmation. MariaDB documents its [compatibility and security tradeoffs](https://mariadb.com/docs/server/reference/plugins/authentication-plugins/authentication-plugin-mysql_native_password); it is not a universal recommendation for new accounts.
...
Tabular results also have 10,000-row and 100,000-cell limits. **SQL Diagnostics** explains common connection, permission, syntax, foreign-key, collation, duplicate-key, and transaction errors. After a failure, read the explanation and suggested checks beside that query or file. It describes the last error, not a guarantee about a newly selected database.

For a schema-related error, select a database instead of Global connection, optionally enter a table name, then inspect its schema. The summary explains the visible table/column counts and collations; expand metadata details for exact types, character sets, and storage engines. Foreign-key comparisons require a specific table. Results are bounded to 500 columns/foreign-key pairs and do not read private row data. Inspection does not change anything, repair SQL, or retry a failed script. A failed script may have committed earlier statements, so check existing changes before rerunning it. Different collations across a database are not by themselves proof of a defect; see MariaDB's [collation guidance](https://mariadb.com/docs/server/reference/data-types/string-data-types/character-sets/setting-character-sets-and-collations) and [foreign-key requirements](https://mariadb.com/docs/server/ha-and-performance/optimization-and-tuning/optimization-and-indexes/foreign-keys).

Remote database connections require TLS with server-certificate verification. App operations use an isolated credential option file, so ambient MariaDB client settings cannot inject commands or enable continue-on-error behavior. Custom CA files and client-certificate configuration are not currently exposed in the app; use a separately configured database client when your server requires them. There is no automatic insecure retry. Explicit localhost or numeric loopback connections retain local compatibility. See [MariaDB client TLS options](https://mariadb.com/docs/server/clients-and-utilities/mariadb-client/mariadb-command-line-client).
...
Returning to the browser restores the selected database, table, view, filters, sort order, and page. Recent rows, table structure, and table statistics are reused for up to 30 seconds; database and table lists are reused for up to two minutes. This bounded cache is memory-only and is cleared when the login or workspace changes. Database-changing operations invalidate cached results, even when they fail partway through. Use the browser's Refresh button to fetch current lists and the selected view immediately, including changes made outside the app. Columns and Indexes do not fetch rows until you open Rows.
